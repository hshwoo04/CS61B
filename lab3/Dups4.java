...
public class Dups4 {
    ...
    static List<String> duplicates(List<String> L) {
        ...
        for (ListIterator<String> p1 = L.listIterator(); p1.hasNext();
             p1.nextIndex()) {
            String x = p1.next();
            if (result.contains(x)== true) {
                continue;
            }
            for (ListIterator<String> p2 = L.listIterator(L.size());
                 p2.previousIndex > p1.nextIndex(); p2.previousIndex()) {
        ...
        return Arraylist.tostring(Arraylist.toarray(result));
    }
    ...
    static void writeList(List<String> L, PrintStream output) {
         ...
         *     }
         */
        output = Collections.sort(output);
        for (String x : L) {
            output.printf("%s ", x);
        }
        output.println();
