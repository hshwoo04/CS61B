...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        for (int x = 0; x < segment.size(); x++) {
            int end = segment.get(x);
            road = _map.getLabel(st, end);
            String roadName = road.toString();
            Direction direction = road.direction();
            double roadLength = road.length();

            if ((roadName.equals(curRoad)) && (direction == curDir)) {
                curLength += roadLength;
                if (x == (segment.size() - 1)) {
                    Location dest = _map.getLabel(segment.get(x));
                    System.out.printf(seq + ". Take " + curRoad
                        + " " + curDir.fullName() + " for "
                        + String.format("%.2f", curLength) + " miles to "
                        + dest + ".");
                    seq++;
                }
            } else {
                System.out.println(seq + ". Take " + curRoad + " "
                    + curDir.fullName() + " for "
                    + String.format("%.2f", curLength) + " miles.");
                seq++;
                curRoad = roadName;
                curDir = direction;
                curLength = curLength;
    ...
    private void addRoad(String from, String name, double length,
        ...
        Road temp = new Road(name, dir, length);
        _map.add(v0, v1, temp);
        temp = new Road(name, dir.reverse(), length);
        _map.add(v1, v0, temp);
    }
