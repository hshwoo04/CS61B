...
class Rule {
    ...
    private void checkFinishedDependencies() {
        for (int y : _depends.successors(_vertex)) {
            Rule x = _depends.getLabel(y);
            if (x.isUnfinished()) {
                throw new IllegalStateException("Not all dependencies exist!");
            }
        }
    ...
    private boolean outOfDate() {
        if (_time < _maker.getCurrentTime()) {
            return true;
        }
        return false;
    }
