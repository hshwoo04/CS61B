...
class CommandInterpreter {
    ...
    Table tableDefinition() {
        ...
        if (_input.nextIf("(")) {
            while (!_input.nextIs(")")) {
                _input.next(",");
                String columnName = _input.next();
                strings.add(columnName);
            }
            _input.next(")");
            table = new Table(strings);
        } else {
            _input.next("as");
            table = selectClause();
        }
    ...
    Table selectClause() {
        ...
        if (_input.nextIs(",")) {
            ...
            _input.next("where");
            conditions = conditionClause(table, table2);
            result = table.select(table2, columnArraylist, conditions);
        } else {
            _input.next("where");
            conditions = conditionClause(table);
            result = table.select(columnArraylist, conditions);
        }
        return result;
    }
    ...
    outer:
        while (!_input.nextIs("and")) {
            Condition condition = condition(tables);
    ...
    Condition condition(Table... tables) {
        Condition condition;
        String column = columnName();
        Column col1 = new Column(column, tables);
        String relation = _input.next(Tokenizer.RELATION);
        if (_input.nextIs(Tokenizer.LITERAL)) {
            String second = literal();
            condition = new Condition(col1, relation, second);
            return condition;
        } else {
            String secondColumn = columnName();
            Column second = new Column(secondColumn, tables);
            condition = new Condition(col1, relation, second);
            return condition;
        }
    }
