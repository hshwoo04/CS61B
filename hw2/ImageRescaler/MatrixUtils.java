...
public class MatrixUtils {
    ...
    public static double compareRow(double[][] table, int row, int column){

        double minimumValue;
        if (row == 0){
            minimumValue = table[row][column];
        }
        else if (column == 0){
            double temp2 = table[row-1][column];
            double temp3 = table[row-1][column+1];
            minimumValue = Math.min(temp2,temp3);
        }
        else if (column == table[row].length){
            double temp1 = table[row-1][column-1];
            double temp2 = table[row-1][column];
            minimumValue = Math.min(temp1,temp2);
        }
        else{
        ...
        minimumValue = Math.min(Math.min(temp1,temp2),temp3);
        }

        return minimumValue;
    }

    public static double[][] accumulateVerticalTemp(double[][] table){
        double[][] temp = table;
        int row = 0;
        int column = 0;
        while (row < table.length){
            while (column < table[row].length){
                temp[row][column] = compareRow(temp, row, column) + temp[row][column];
                column++;
            }
            row++;
        }
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        return accumulateVerticalTemp(m);
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        ...
        int j = m[0].length;
        double[][] temp;
        if (orientation == MatrixUtils.Orientation.HORIZONTAL){
            temp = new double[j][i];
            for (int num = 0; num < i; num++){
                for (int num1 = 0; num1 < j; num1++){
                    temp[num1][num] = m[num][num1];
                }
            }
        }
        else{
