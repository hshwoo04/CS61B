...
class Game extends Observable {
    ...
    private void checkForWin() {
        if (gameInProgress() && _board.getWinner() != null) {
            announce();
            _playing = false;
        }
    }
    ...
    private void setSpots(int r, int c, int spots, String color) {
        try {
            assert (spots <= _board.neighbors(r, c));
            if (spots == 0) {
    ...
    private void readExecuteCommand() {
        try {
            if (_inp.hasNextLine()) {
                if (_inp.hasNextInt() && gameInProgress()) {
                    int one = _inp.nextInt();
                    if (_inp.hasNextInt()) {
                        int two  = _inp.nextInt();
                        saveMove(one, two);
    ...
    /** Checks if _inp has a next line and calls it.*/
    private void nextLineMethod() {
        if (_inp.hasNextLine()) {
            _inp.nextLine();
        }
    }
