...
abstract class Board extends Observable {
    ...
    boolean isLegal(Side player, int n) {
        Square sq = get(row(n), col(n));
        if (sq.getSide() == WHITE || sq.getSide() == player) {
            return true;
        }
        return false;
    }
    ...
    final Side getWinner() {
        int num = size() * size();
        if (numOfSide(RED) == num) {
            return RED;
        } else if (numOfSide(BLUE) == num) {
            return BLUE;
        } else {
            return null;
        }
